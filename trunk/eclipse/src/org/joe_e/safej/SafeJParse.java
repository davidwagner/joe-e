...
public class SafeJParse {   
    ...
    int nextToken;
    PrintStream err;
    SafeJConsumer consumer;
    
    String comment = null; // the class comment
    List<String> honoraries = new ArrayList<String>();
    List<Member> staticMembers = new ArrayList<Member>();
    List<Member> instanceMembers = new ArrayList<Member>();
    
    public SafeJParse(File dir, PrintStream err, SafeJConsumer consumer) {
        this.err = err;
        this.dir = dir;
    ...
    private void process(final File f) {
        ...
        } else if (f.getName().endsWith(".safej")) {
            honoraries.clear();
            staticMembers.clear();
    ...
    private void failParsing(String expectation) throws ParseFailure {
        err.println(currentFile + ":" + st.lineno() + ": " + expectation);
